...
find "$RESTORED_DIR" -name "*.html" -not -path "*/backup_*/*" -exec cp {} "$RESTORED_DIR/backup_$(date +%Y%m%d)/" \;
...
for html_file in "$RESTORED_DIR"/*.html; do
  ...
  # Check if the animation references already exist in the file
  if ! grep -q "title-animation.css" "$html_file"; then
    ...
    echo "Already contains animation references: $html_file"
  fi
done
